...
int push(int * mem, int val)	{
	returnstack++;
	if (returnstack>999) return 1;
	(*(mem+999-returnstack)) = val;
	return 0;
}

int pop(int * mem)	{
	int temp = *(mem+999-returnstack);
	returnstack--;
	printf("ret to %d\n",temp);
	return temp;
}
	...
	printf("\trunning %d\n",ip);
		...
		switch (c)      {
			...
			case 'r':	{
				...
				else if (ipjump>=vpjump)	{
					ip = pop(mem);//to prevent jump loops
					ipjump--;
					break;
				}
